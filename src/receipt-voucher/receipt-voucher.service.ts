...
export class ReceiptVoucherService {
  ...
  async updateMultipleReceiptVouchers(
    ...
    const updatedReceiptVouchers: ReceiptVoucher[] = [];

    for (const transaction of transactions) {
      const { receiptVoucherId, customerAccountId, date, invoiceId, details } =
        transaction;

      // Find the existing Receipt Voucher
      const existingVoucher = await this.receiptVoucherRepository.findOne({
      ...
      if (!existingVoucher) {
        throw new NotFoundException(
          `Receipt Voucher with ID ${receiptVoucherId} not found.`,
        );
      }
      ...
      await this.receiptVoucherDetailRepository.delete({
        receiptVoucher: { id: receiptVoucherId },
      });

      // Process new details
      const updatedDetails = details.flatMap((detail) => {
        const isUSD = detail.currency === 'USD';
        const cashNumber = parseFloat(detail.cashNumber);
        ...
        const creditDetail = this.receiptVoucherDetailRepository.create({
          ...
          receiptVoucher: existingVoucher,
        });
      ...
      existingVoucher.customer = customer;
      existingVoucher.date = date;
      existingVoucher.invoiceId = invoiceId;
      existingVoucher.totalDr = totalDr;
      existingVoucher.totalDrUSD = totalDrUSD;
      existingVoucher.totalDrLL = totalDrLL;
      existingVoucher.totalCr = totalCr;
      existingVoucher.totalCrUSD = totalCrUSD;
      existingVoucher.totalCrLL = totalCrLL;
      existingVoucher.details = updatedDetails;

      await this.receiptVoucherRepository.save(existingVoucher);
      await this.receiptVoucherDetailRepository.save(updatedDetails);

      updatedReceiptVouchers.push(existingVoucher);
    }
